...
const statusErrMsg = "status = %d, want %d"
...
func TestJWTValidatorValidateAccessToken(t *testing.T) {
	validator := middleware.NewJWTValidator(testSecret)
	userID := uuid.New().String()

	t.Run("valid token", func(t *testing.T) {
		tok := makeToken(t, testSecret, userID, testEmail, 15*time.Minute)
	...
	t.Run("expired token", func(t *testing.T) {
		tok := makeToken(t, testSecret, userID, testEmail, -1*time.Minute)
	...
	t.Run("wrong secret", func(t *testing.T) {
		tok := makeToken(t, "wrong-secret", userID, testEmail, 15*time.Minute)
		_, _, err := validator.ValidateAccessToken(tok)
		if err == nil {
	...
	t.Run("wrong signing method rejected", testWrongSigningMethod)
}
...
func TestAuthMiddleware(t *testing.T) {
	...
	t.Run("valid bearer token passes", func(t *testing.T) {
		...
		if w.Code != http.StatusOK {
			t.Errorf(statusErrMsg, w.Code, http.StatusOK)
		}
	})
	...
	t.Run("missing authorization header returns 401", func(t *testing.T) {
		...
		if w.Code != http.StatusUnauthorized {
			t.Errorf(statusErrMsg, w.Code, http.StatusUnauthorized)
		}
	})
